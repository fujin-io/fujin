...
func TestConvertWriterConfigValue(t *testing.T) {
	tests := []struct {
		...
		// Note: Validation of allowed paths is now done by ValidateOverridePath,
		// not by the converter. The converter only converts values.
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := convertConfigValue(tt.settingPath, tt.value)
			if tt.wantErr {
				if err == nil {
