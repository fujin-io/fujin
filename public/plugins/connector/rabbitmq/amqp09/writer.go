...
func NewWriter(conf ConnectorConfig, l *slog.Logger) (*Writer, error) {
	if conf.Publish == nil {
		return nil, fmt.Errorf("rabbitmq_amqp09: publish config is required")
	}
	...
	if err != nil {
		_ = conn.Close()
		return nil, fmt.Errorf("rabbitmq_amqp09: open channel: %w", err)
	}
