...
func NewReader(conf ConnectorConfig, autoCommit bool, l *slog.Logger) (connector.ReadCloser, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("resp_streams: get tls config: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("resp_streams: new client: %w", err)
	}
		...
		for stream, streamConf := range r.conf.Streams {
			if err := client.Do(
				...
				if !rueidis.IsRedisBusyGroup(err) {
					return nil, fmt.Errorf("resp_streams: xgroup create: %w", err)
				}
			}
		}
			...
			r.handler = func(
				...
				for stream, msgs := range resp {
					...
					for _, msg = range msgs {
						...
						if err != nil {
							r.l.Error("resp_streams: failed to marshal message", "error", err)
							continue
						}
						h(data, stream)
					}
	...
	for {
		...
		default:
			...
			if err != nil {
				...
				return fmt.Errorf("resp_streams: xread: %w", err)
			}
...
func (r *Reader) Fetch(
	...
	if err != nil {
		...
		fetchHandler(0, fmt.Errorf("resp_streams: xread: %w", err))
		return
	}
