...
// convertWriterConfigValue converts and validates a configuration value for Kafka writer
func convertWriterConfigValue(settingPath string, value string) (any, error) {
	...
	if !allowedWriterSettings[settingPath] {
		// Check if it's a nested TLS setting
		if strings.HasPrefix(settingPath, "tls.") {
			return nil, fmt.Errorf("setting '%s' cannot be overridden", settingPath)
		}
		return nil, fmt.Errorf("setting '%s' cannot be overridden", settingPath)
	}
...
// convertReaderConfigValue converts and validates a configuration value for Kafka reader
