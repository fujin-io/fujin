...
var allowedWriterSettings = map[string]bool{
	// Currently no runtime-overridable settings for NATS Core writer
	// URL and Subject are connection/topic identifiers and should not be changed at runtime
}

// allowedReaderSettings contains settings that can be overridden at runtime
// Only settings listed here can be dynamically configured per session
var allowedReaderSettings = map[string]bool{
	// Currently no runtime-overridable settings for NATS Core reader
	// URL and Subject are connection/topic identifiers and should not be changed at runtime
}

// convertWriterConfigValue converts and validates a configuration value for NATS Core writer
...
// convertReaderConfigValue converts and validates a configuration value for NATS Core reader
